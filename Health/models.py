...
class Health(models.Model):
    ...
    slug            = models.SlugField(null = True, max_length=1000,blank=False)
...
def pre_save_post_receiver(sender, instance, *args, **kwargs):
    ...
    exist       = Health.objects.filter(slug = slug).exists()
    if exist:
...
pre_save.connect(pre_save_post_receiver, sender = Health)
